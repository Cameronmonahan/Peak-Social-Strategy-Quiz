...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
const FORM_ENDPOINT = "https://formspree.io/f/mvkpdvka";
...
function showResult(resultKey){
  ...
  const summary = [
    `RESULT: ${r.attention}% Attention / ${r.identity}% Identity  (${r.strategy})`,
    ``,
    `FULL ANSWER PATH`,
    answerPath,
    ``,
  ...
  document.getElementById("hiddenResult").value = `${r.strategy}`;

  // Raw structured fields (kept for spreadsheet export / automation)
  ...
  document.getElementById("hiddenPath").value = answerPath;
  document.getElementById("hiddenDate").value = completedAt;
...
leadForm.addEventListener("submit", async (e) => {
  ...
  const leadName = leadForm.querySelector('[name="name"]').value.trim();
  const leadCompany = leadForm.querySelector('[name="company"]').value.trim();
  const resultLabel = document.getElementById("hiddenResult").value;
  document.getElementById("hiddenSubject").value =
    `New Lead: ${resultLabel} — ${leadCompany || leadName || "PEM Strategy Finder"}`;
